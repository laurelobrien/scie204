...
// 20151026
//
// Draw a controllable hero on 
// rotating tiled background
/////////////////////////////

//initialize and declare colour variables for global use
//colour indicated by variable is self-explanatory (eg lightGrey) 
...
void draw()
{
  ...
  //only draw bard when mouse is in top half of canvas
  if (mouseY <= height/2) //if mouse is in top half of canvas:
  {
    rotateBard(); //call function that rotates drawBard() to an angle based on mouse x position
  }
...
void drawGlobeTileGrid()
{
  ...
  //for 16 columns of tiles:
  for (int j = 0; j < 17; j ++)
  {
    //draw 16 rows of tiles
    for (int i = 0; i < 17; i ++)
    {
